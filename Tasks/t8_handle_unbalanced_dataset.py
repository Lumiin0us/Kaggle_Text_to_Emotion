...
#     if len(twitter_df.loc[twitter_df['sentiment'] == sentiment]['content']) < 1500:
#         # print(sentiment, len(twitter_df.loc[twitter_df['sentiment'] == sentiment]['content']))
#         pass
#         # print(sentiment, twitter_df.loc[twitter_df['sentiment'] == sentiment]['content'], end=', ')

# #caculating Wu & Palmer similarity
# #! we have picked the maximum similarity score we could find among all synsets of the particular labels, another approach we could have taken was to 
# #! take the average score over all synsets which may give us a better idea? 
# similarities = {}
...
# def similarity_table(similarities):
#     df_table = pd.DataFrame(similarities, index=['Best Matching Synsets', 'Best Matching Synsets', 'Maximum Similarity', 'Mean Similarity'])
#     print(df_table)

# calculate_similarities(all_sentiments=all_sentiments)
# similarity_table(similarities=similarities)

# def merging_categories(similarities_dict):
#     frequency_thresold = 1500
#     merging_thresold = 0.4
#     small_categories = []
#     merging_list = []
#     merge_categories_matched = []
#     merge_mean = []
#     for sentiment in all_sentiments:
#         if len(twitter_df.loc[twitter_df['sentiment'] == sentiment]) < frequency_thresold:
#             small_categories.append(sentiment)
#     print('small category', small_categories)
#     for key, value in similarities_dict.items():
#         if key[0] in small_categories and key[1] in small_categories and value[-2] >= merging_thresold:
#             print(key)
#             merge_categories_matched.extend(key)
#             merge_mean.append(value[-1])
#             merging_list.append(key)
...
#             if key[0] in small_categories and key[1] in small_categories and value[-2] >= merging_thresold and values in key:
#                 total_count += val[-1]
...
#     for key, val in similarities_dict.items():
#             if key[0] in small_categories and key[1] in small_categories and value[-2] >= merging_thresold and merging_cat in key:
#                 print('kk', key)
#                 labels_to_merged_with_merging_cat.extend(key)
#     labels_to_merged_with_merging_cat = list(set(labels_to_merged_with_merging_cat))
#     labels_to_merged_with_merging_cat.remove('anger')
#     print(labels_to_merged_with_merging_cat)
...
#     # print(new_sentiment_df.columns.values)
#         new_sentiment_df.drop(['sentiment', 'content', 'tweet_id'], axis=1, errors='ignore', inplace=True)

#     for col_index in range(1, len(labels_to_merged_with_merging_cat)):
#         # new_sentiment_df['sentiment']
#         pd.concat([new_sentiment_df[labels_to_merged_with_merging_cat[0]], new_sentiment_df[labels_to_merged_with_merging_cat[col_index]]], axis=1, ignore_index=True, )
#         new_sentiment_df.drop([labels_to_merged_with_merging_cat[col_index]], axis=1, inplace=True)
#     # print(keys_with_max_count)
#     # print(Counter(merge_categories_matched).most_common())
#     # print(merging_list)
#     print(new_sentiment_df['anger'])

# merging_categories(similarities_dict=similarities)
