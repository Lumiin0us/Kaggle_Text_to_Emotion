...
# Calculate GPT-2 embeddings for each record or tweet of our sentiment (cat_1, cat_2) and store them in "sentiment_vector_map"
def calculate_gpt2_embeddings_for_each_record(cat_1, cat_2):
...
def calculate_gpt2_embeddings_for_overall_list(cat_1, cat_2):
    sentiment_vector_map = {}
...
# Calculate cosine similarities between GPT-2 embeddings
def calculate_cosine_similarities(gpt2_embeddings_words, gpt2_embeddings_list, cat_1, cat_2):
    similarities = []
    all_sentiments = [cat_1, cat_2]

    for sentiment in all_sentiments:
        embeddings = gpt2_embeddings_words[sentiment]
        mean_embedding = np.mean(embeddings, axis=0)

        for other_sentiment in all_sentiments:
            other_embeddings = gpt2_embeddings_list[other_sentiment]
...
def record_based_similarity_between_each_pair(all_sentiments):
    ...
    for pairs in combinations(all_sentiments, 2):
        gpt2_embeddings_words = calculate_gpt2_embeddings_for_each_record(pairs[0], pairs[1])
        gpt2_embeddings_list, cat_1, cat_2 = calculate_gpt2_embeddings_for_overall_list(pairs[0], pairs[1])

        similarities = calculate_cosine_similarities(gpt2_embeddings_words, gpt2_embeddings_list, cat_1, cat_2)
